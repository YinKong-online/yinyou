...
# 加载音频文件
filename = r"C:\Users\lenovo\Desktop\剑与雪.mp3"
y, sr = librosa.load(filename)

# 将 mp3 转换为 wav
sf.write("剑与雪.wav", y, sr)
...
def play_audio():
    global start_time
    pygame.mixer.music.load("剑与雪.wav")
    pygame.mixer.music.play()
    start_time = datetime.now().timestamp()

# 记录当前时间的函数
def record_time(event=None):
...
# 绑定任意按键事件到 record_time 函数
root.bind("<KeyPress>", record_time)
